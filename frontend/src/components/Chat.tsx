...
export default function Chat() {
  ...
  const speakMessage = async (content: string, messageIndex: number) => {
    ...
    if (playingMessageId === messageIndex.toString()) {
    ...
    try {
      ...
      addTTSLoadingMessage(messageIndex.toString())
      ...
      if (isBrowserTTS) {
        ...
        removeTTSLoadingMessage(messageIndex.toString())
        ...
        if ('speechSynthesis' in window) {
          ...
          utterance.onstart = () => {
            setPlayingMessageId(messageIndex.toString())
          }
                    ...
                    {message.role === 'assistant' && (
                      <Button
                        ...
                        className={`absolute top-1 left-1 h-6 w-6 p-0 transition-all duration-200 bg-background/50 hover:bg-background ${
                          isMessageTTSLoading(index.toString()) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                        }`}
                        onClick={() => speakMessage(message.content, index)}
                        title={
                          isMessageTTSLoading(index.toString())
                            ? "Generating TTS..."
                            : playingMessageId === index.toString()
                              ? "Stop TTS playback"
                              : "Generate and play TTS"
                        }
                        disabled={isMessageTTSLoading(index.toString())}
                      >
                        {isMessageTTSLoading(index.toString()) ? (
